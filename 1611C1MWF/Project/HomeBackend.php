...
if(!empty($_POST["hid"]))
{
	if(empty($_FILES["fileupload"]["name"]))
	{
		$uf = $_POST["path"];
		$ehid = $_POST["hid"];
		$update = "update home_table set Picture='".$uf."', Description='".$un."' where Id='".$ehid."'";
	...
	if(!empty($_FILES["fileupload"]["name"]))
	{
		...
		{
			$ehid = $_POST["hid"];
			$update = "update home_table set Image='".$file."', Description='".$un."' where Id='".$ehid."'";
			$exe = mysqli_query($connection,$update);
			if($exe)
			{
				$result = "Registration Successfully Updated";
...
if(!empty($_FILES["fileupload"]["name"]))
{
	if(move_uploaded_file($tmp_name,$location.$file))
	{
		$insert = "insert into home_table (Picture,Description) values('".$file."','".$un."')";
...
if(isset($_GET["e_id"]))
{
	...
	$eid = $_GET["e_id"];
	$select = "select * from home_table where Id = '".$eid."'";
                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            ...
                            <div class="row">
                            		<div class="col-md-8">
										<form action="HomeBackend.php" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                    	<label for="exampleInputEmail1">Enter Description</label>
                                         <textarea class="form-control" id="inputName" name="name"><?php echo @$name ?></textarea>
									</div>
