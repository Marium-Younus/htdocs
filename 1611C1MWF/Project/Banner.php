...
if(!empty($_POST["hid"]))
{
	if(empty($_FILES["fileupload"]["name"]))
	{
		$uf = $_POST["path"];
		$ehid = $_POST["hid"];
		$update = "update banner_table set Banner_Picture='".$uf."' where Banner_Id='".$ehid."'";
		$exe = mysqli_query($connection,$update);
		if($exe)
		{
			$result = "Banner Successfully Updated";
		}
		else
		{
			$result = "Banner Unsuccessfully Updated";
		}
	...
	if(!empty($_FILES["fileupload"]["name"]))
	{
		...
		{
			$ehid = $_POST["hid"];
			$update = "update banner_table set Banner_Picture='".$file."' where Banner_Id='".$ehid."'";
			$exe = mysqli_query($connection,$update);
			if($exe)
			{
				$result = "Banner Successfully Updated";
...
if(!empty($_FILES["fileupload"]["name"]))
{
	if(move_uploaded_file($tmp_name,$location.$file))
	{
		$insert = "insert into banner_table (Banner_Picture) values('".$file."')";
		$exe = mysqli_query($connection,$insert);
		if($exe)
		{
			$result = "Banner Insert Successfully";
		}
		else
		{
			$result = "Banner Insert Unsuccessfully";
		}
	}
}
}
...
if(isset($_GET["e_id"]))
{
	...
	$eid = $_GET["e_id"];
	$select = "select * from banner_table where _Banner_Id = '".$eid."'";
	$query = mysqli_query($connection,$select);
	$row = mysqli_fetch_array($query);
	$img = $row["Banner_Picture"];
}	
if(!isset($_GET["e_id"]))
{
	$bt = "Insert";
